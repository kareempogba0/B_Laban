...
class CouponService {
    ...
    try {
      ...
      if (isProductSpecific) {
        ...
        if (coupon.minOrderAmount > 0 && eligibleItemsTotal < coupon.minOrderAmount) {
          ...
          return {
            valid: false,
            message: `This coupon requires eligible products total of ${coupon.minOrderAmount.toLocaleString()} EGP.`,
            coupon: null
          };
        }
      ...
      if (coupon.minOrderAmount > 0 && cartTotal < coupon.minOrderAmount) {
        ...
        return {
          valid: false,
          message: `This coupon requires a minimum order of ${coupon.minOrderAmount.toLocaleString()} EGP.`,
          coupon: null
        };
      }
  ...
  static formatDiscount(coupon, discountAmount, isProductSpecific = false) {
    ...
    if (coupon.discountType === "percentage") {
      return `${coupon.discountValue}% OFF${productText} (${discountAmount.toLocaleString('en-EG', {
        maximumFractionDigits: 2,
        minimumFractionDigits: 2
      })}) EGP`;
    } else {
      return `${discountAmount.toLocaleString('en-EG', {
        maximumFractionDigits: 2,
        minimumFractionDigits: 2
      })} OFF${productText} EGP`;
    }
