...
function OrderSummary() {
  ...
  useEffect(() => {
    ...
    const fetchOrderDetails = async () => {
      if (!orderId) {
        setError("Invalid order link. No order ID provided.");
        setLoading(false);
        return;
      }
  ...
  const formatDate = (dateSource) => {
    ...
    const date = dateSource.seconds ? new Date(dateSource.seconds * 1000) : new Date(dateSource);
                ...
                <div className="divide-y divide-gray-200">
                  <AnimatePresence>
                    {order?.items?.map((item, index) => (
                      ...
                      >
                        {/*Image*/}
                        <div className="flex-shrink-0 w-16 h-16 border border-gray-200 rounded-md overflow-hidden bg-gray-100 flex items-center justify-center">
                          {(item.image || item.imageUrl) ? (
                              <img
                                  ...
                                  onError={(e) => {
                                    console.error(`[Image Error] Failed to load image from URL: ${e.target.src}`);
